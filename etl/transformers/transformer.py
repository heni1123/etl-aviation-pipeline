...
class Transformer:
    ...
    def _br1_altitude_category(self, row: Dict[str, Any]) -> str:
        if row.get('baro_altitude') is None or row.get('on_ground') == True:
            return 'Ground'
    ...
    def _br3_speed_category(self, row: Dict[str, Any]) -> str:
        if row.get('velocity') is None or row.get('on_ground') == True:
    ...
    def _br4_emergency_flag(self, row: Dict[str, Any]) -> str:
        ...
        elif row.get('spi') == True:
            return 'Special Purpose'
        return 'Normal'

    def _br5_data_quality_flag(self, row: Dict[str, Any]) -> str:
        return 'Valid' if row.get('icao24') is not None else 'Invalid'